...
export function JobPostForm() {
  ...
  const preview: JobPreviewData = {
    ...
    travel_percent: form.travel_percent,
    therapeutic_area: form.therapeutic_area,
    trial_phase: form.trial_phase,
    salary_range: form.salary_range,
    start_date: form.start_date,
    is_junior_friendly: form.is_junior_friendly,
    target_audience: [
      ...(form.target_cra ? ["student_cra"] : []),
      ...(form.target_gcp ? ["student_gcp"] : []),
    ],
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          >
            ...
            <Field label="כותרת המשרה" required>
              <input name="title" value={form.title} onChange={set("title")} style={inputStyle} placeholder="לדוגמה: CRA ג'וניור" required />
            </Field>

            <div className="grid grid-cols-2 gap-4">
              <Field label="שם החברה / הארגון" required>
                <input
                  name="company"
                  value={form.is_discrete ? "" : form.company}
                  onChange={set("company")}
                  style={{ ...inputStyle, opacity: form.is_discrete ? 0.4 : 1 }}
                  placeholder="שם החברה"
                  disabled={form.is_discrete}
                  required={!form.is_discrete}
                />
              </Field>
              <Field label="דיסקרטי">
            ...
            <Field label="תיאור המשרה" required>
              <textarea
                ...
                placeholder="תארו את התפקיד, הצוות והסביבה..."
                required
              />
            </Field>
            ...
            <Field label="דרישות סף">
              <textarea
                ...
                placeholder="תואר ראשון במדעי החיים, אנגלית ברמה גבוהה..."
              />
            </Field>
            ...
            <Field label="דרך הגשת מועמדות" required>
              <input
                ...
                style={inputStyle}
                placeholder="careers@company.com או לינק לאתר"
                required
              />
            </Field>
          </div>
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label="אחוז נסיעות">
                <input
                  type="number"
                  name="travel_percent"
                  value={form.travel_percent}
                  onChange={set("travel_percent")}
                  style={inputStyle}
                  placeholder="0-100"
                  min={0}
                  max={100}
              ...
              <Field label="טווח שכר">
                <input
                  ...
                  style={inputStyle}
                  placeholder="לדוגמה: 12,000-18,000 ₪"
                />
              </Field>
            </div>
            ...
            <Field label="תאריך התחלה משוער">
              <input
                ...
                style={inputStyle}
                placeholder="לדוגמה: מיידי, מרץ 2026"
              />
            </Field>
          </div>
          ...
          >
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <div>
                ...
                <div style={{ color: "#64748b", fontSize: 12 }}>יוצג תג מיוחד שמושך בוגרים חדשים</div>
              </div>
            </label>
          </div>
